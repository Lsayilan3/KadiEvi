...
import { getevDetailDilId, getOdaEkServiceDilId, getOdaOlanakDilId } from "../../services/housesServices/houseService";
import { FormattedMessage } from "react-intl";

function RoomFeatures({ evId ,name}) {
  const [evData, setEvData] = useState(null);
  const [evDetail, setEvDetail] = useState(null);
  const [ekServisler, setEkServisler] = useState([]);
  const [olanaklar, setOlanaklar] = useState([]);
  ...
  const fetchEvData = async (evId, dil) => {
    try {
      const [evDetail, ekServisData, olanakData] = await Promise.all([
        getevDetailDilId(evId, dil),
        getOdaEkServiceDilId(evId, dil),
        getOdaOlanakDilId(evId, dil),
      ]);
  ...
  return (
    <>
      ...
      {evDetail && evDetail.map((evDetail, index) => (
  <>
      <h2>{evDetail.baslik}</h2>
...
/>
      <p>{evDetail.p}</p>
      <h4>{evDetail.cocukBaslik}</h4>
      <p>{evDetail.cocukP}</p>


      </>
   ))}
      <h4><FormattedMessage id="oda_ozelligi" values={{ name }}/></h4>
      <ul className="room-features d-flex align-items-center">
  {ekServisler.map((ekServis, index) => (
    <li key={index}>
      <span>{ekServis.baslik}</span>

      {/* Dinamik SVG'yi render et */}
...
<path d="M15.6739 13.5293C15.5801 13.5879 14.2793 14.8594 12.791 16.3535C10.9746 18.1758 10.0723 19.125 10.043 19.2363C9.99612 19.4297 10.1074 19.7168 10.2774 19.834C10.3418 19.8809 10.5059 19.9219 10.6348 19.9219C10.8633 19.9219 10.9043 19.8867 13.6934 17.0918C16.4356 14.3555 16.5235 14.2617 16.5235 14.0449C16.5235 13.7578 16.4473 13.6172 16.2305 13.5059C16.0078 13.3887 15.8848 13.3945 15.6739 13.5293Z"></path>
<path d="M8.30859 20.8945C8.10352 21 8.02734 21.1465 8.02734 21.416C8.02734 21.7969 8.43164 22.0605 8.79492 21.9141C9.02344 21.8203 9.14062 21.6445 9.14062 21.3984C9.14062 21.1406 9.05859 21 8.84766 20.8887C8.63672 20.7773 8.51367 20.7832 8.30859 20.8945Z"></path>
<path d="M24.885 8.75977C24.5217 9.12891 24.7326 9.67383 25.2775 9.75586C25.4123 9.7793 26.0041 9.78516 26.5842 9.77344C27.551 9.75586 27.6623 9.74414 27.8088 9.63281C28.1135 9.41016 28.1369 9.0293 27.8615 8.75391C27.7209 8.61328 27.7209 8.61328 26.3791 8.61328H25.0373L24.885 8.75977Z"></path>
<path d="M25.0374 11.0391C24.6624 11.209 24.592 11.6895 24.8967 11.9648L25.0784 12.1289H26.385C27.5276 12.1289 27.7034 12.1113 27.8088 12.0234C28.1135 11.7832 28.1311 11.3848 27.8557 11.127C27.7092 10.9863 27.6858 10.9863 26.467 10.9746C25.4944 10.9629 25.1838 10.9746 25.0374 11.0391Z"></path>
<path d="M24.8964 13.5293C24.7031 13.7227 24.662 14.0625 24.8144 14.2734C24.9784 14.5137 25.1659 14.5371 26.4726 14.5195C27.6796 14.502 27.7089 14.4961 27.8554 14.3613C28.037 14.1856 28.1015 13.9336 28.0077 13.7051C27.873 13.3887 27.7323 13.3594 26.3378 13.3594H25.0663L24.8964 13.5293Z"></path>
